...
class SelfRefinementEngine:
    def __init__(self, args, current_round):
        #model = T5ForConditionalGeneration.from_pretrained("google/flan-t5-base")#,force_download=True, resume_download=False)
        #tokenizer = AutoTokenizer.from_pretrained("google/flan-t5-base")#,legacy=False,use_fast=False)
        self.args = args
    ...
    def load_logic_programs(self):
        ...
        with open(os.path.join('../outputs/logic_programs', f'{prefix}{self.dataset_name}_{self.split}.json')) as f:
            dataset = json.load(f)
    ...
    def single_round_self_refinement(self):
        ...
        for example in tqdm(self.logic_programs):
            ...
            if status == 'execution error':
                if not error_message == 'No Output': # this is not execution error, but parsing error
                    ...
                    revised_program = tokenizer.decode(temp[0], skip_special_tokens=True)
        ...
        # save outputs
        save_path = f'../outputs/logic_programs/self-refine-{self.current_round}_{self.dataset_name}_{self.split}.json'
